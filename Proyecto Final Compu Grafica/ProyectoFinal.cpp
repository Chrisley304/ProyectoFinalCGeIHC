...
int main()
{
	...
	bool cambioMusica = true;
	bool cambioMusicaR = true;
	bool sonido = true;
	// Loop mientras no se cierra la ventana
	while (!mainWindow.getShouldClose()) {
#ifndef __APPLE__
		if (cambioMusica != mainWindow.getMusica()) {
			if (sonido) {
				engine->play2D("Sounds/DaleZeldaDale.wav", true);
				sonido = false;
				cambioMusica = !cambioMusica;
				printf("Activando musica\n");
			}
		}
		
		if (cambioMusicaR != mainWindow.getMusicaR()) {
			if (!sonido) {
				engine->stopAllSounds();
				sonido = true;
				cambioMusicaR = !cambioMusicaR;
				printf("Desactivando musica\n");
			}
		}
